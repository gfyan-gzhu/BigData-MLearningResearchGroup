    ...
    def train(self):
        ...
        for epoch in range(args.num_epochs):
            ...
            if args.data_name == 'amazon':
                sampled_idx_train = undersample(train_pos, train_neg, scale=2)
            if args.data_name == 't-finance':
                sampled_idx_train = pick_step(
                    idx_train,
                    y_train,
                    self.dataset["homo"],
                    size=len(self.dataset["train_pos"]) * 8,
            ...
            for batch in range(num_batches):
                ...
                loss = gnn_model.loss(batch_nodes, batch_label_tensor)
                # optimizer.zero_grad()
                loss.backward()
        ...
        try:
            ...
            try:
                if self.args.model == 'GNN':
                    ...
                    with torch.no_grad():
                        _, _, _, _, _, _, test_emb = self.gnn_model(test_batch, test_labels)
                else:
                    with torch.no_grad():
                ...
                for i in range(0, num_nodes, batch_size):
                    ...
                    try:
                        if self.args.model == 'GNN':
                            ...
                            _, _, _, _, _, _, emb = self.gnn_model(batch_nodes, batch_labels)
                        else:
