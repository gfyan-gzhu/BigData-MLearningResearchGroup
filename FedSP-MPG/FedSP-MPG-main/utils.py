...
def load_configs(args):
    ...
    # 复制一份，避免直接 pop 原始配置对象
    dataset_configs = dict(configs["datasets"][args.dataset])
    ...
    model_configs = dict(configs["models"].get(
        f"{args.model}_{task_literal}",
        configs["models"][args.model],
    ))
    framework_configs = dict(configs["frameworks"].get(
        f"{args.framework}_{task_literal}",
    ...
    # 只有 FedHGN / FedSP-MPG 保留 ablation 语义
    if args.framework not in ["FedHGN", "FedSP-MPG"]:
        args.ablation = None
    ...
    all_configs.update(framework_configs)

    # 再用 CLI 非空参数覆盖 YAML
    for k, v in vars(args).items():
        if v is not None or k not in all_configs:
            all_configs[k] = v
...
def get_save_path(args, prefix="./saves"):
    # 第一层：实验类别文件夹
    # normal / chaocan / xiaorong
    exp_folder = getattr(args, "exp_folder", "normal")
    ...
    # 如果是 FedSP-MPG，把关键超参也写进去，后面查结果更舒服
    if hasattr(args, "mp_num_gating_bases"):
        exp_parts.append(f"mpb{args.mp_num_gating_bases}")
    if hasattr(args, "mp_max_paths"):
        exp_parts.append(f"mpp{args.mp_max_paths}")

    exp_name = "_".join(exp_parts)

    # 第二层：framework / framework_ablation
    framework_dir = args.framework if args.ablation is None else f"{args.framework}_{args.ablation}"
    ...
    # 同一个实验重复跑时，用 1/2/3/... 区分
    old_saves = [
        int(str(x.name))
        for x in save_path.iterdir()
        if x.is_dir() and str(x.name).isdigit()
    ]
    save_num = 1 if len(old_saves) == 0 else max(old_saves) + 1

    save_path = save_path / str(save_num)
    save_path.mkdir()

    # copy config file to the save dir
