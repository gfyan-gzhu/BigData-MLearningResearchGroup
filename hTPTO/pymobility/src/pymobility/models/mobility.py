...
# define a Uniform Distribution 定义均匀分布
"""
MIN: 随机数的最小值
MAX: 随机数的最大值
SAMPLES: 随机数的数量或形状
"""
U = lambda MIN, MAX, SAMPLES: rand(*SAMPLES.shape) * (MAX - MIN) + MIN  #使随机数的范围位于[MIN, MAX]

# define a Truncated Power Law Distribution 定义截断幂律分布
"""
幂律分布的基础上加入了一个截断条件，即在x趋向于某个值时，幂律分布的幅度会逐渐减小，最终变为一个常数。
ALPHA: 幂律分布的指数
MIN: 随机数的最小值
MAX: 随机数的最大值
SAMPLES: 随机数的数量或形状
...
# define an Exponential Distribution    定义指数分布
"""
SCALE: 指数分布的确界参数,控制分布的衰减速率
SAMPLES: 随机数的数量或形状
利用指数分布CDF的逆转换方法生成随机数:
"""
E = lambda SCALE, SAMPLES: -SCALE*np.log(rand(*SAMPLES.shape))

# *************** Palm state probability状态概率 **********************
"""
计算随机游走模型中节点暂停概率的初始化函数
pause_low: 暂停时间的下限
pause_high: 暂停时间的上限
speed_low: 节点速度下限
speed_high: 节点速度上限
...
def pause_probability_init(pause_low, pause_high, speed_low, speed_high, dimensions):
    alpha1 = ((pause_high+pause_low)*(speed_high-speed_low))/(2*np.log(speed_high/speed_low)) #计算暂停概率的公式
    delta1 = np.sqrt(np.sum(np.square(dimensions))) # 计算区域对角线的长度
    return alpha1/(alpha1+delta1) # 返回计算的暂停概率
...
初始随机节点位置
nr_nodes：节点数量。
dimensions：表示模拟区域的维度。
speed_low 和 speed_high：速度的下限和上限。
pause_low 和 pause_high：暂停时间的下限和上限
"""
def init_random_waypoint(nr_nodes, dimensions,
    ...
    q0 = pause_probability_init(pause_low, pause_high, speed_low, speed_high, dimensions) #计算停顿概率
...
class RandomWaypoint(object):
    ...
    def __init__(self, nr_nodes, dimensions, velocity=(0.1, 1.), wt_max=30):
        '''
        Random Waypoint model.随机路点模型
        
        Required arguments:需要的参数
        
          *nr_nodes*:节点数
            Integer, the number of nodes.整数，节点数量
          
          *dimensions*:维度
            Tuple of Integers, the x and y dimensions of the simulation area.
            整型元组，模拟区域的x,y维度
          
        keyword arguments:关键参数
        
          *velocity*:速度
            Tuple of Integers, the minimum and maximum values for node velocity.
            整型元组，节点速度的最小，最大值
          *wt_max*:停留时间
            Integer, the maximum wait time for node pauses.整型，节点最大停留时间
            If wt_max is 0 or None, there is no pause time.如果wt_max是0或者None，表示没有停留时间
        '''
    ...
    def __iter__(self):
        ...
        # assign nodes' movements (direction * node velocity)——指定节点的移动（方向，节点速度）
...
class RandomDirection(StochasticWalk):
    ...
    def __init__(self, nr_nodes, dimensions, wt_max=None, velocity=(0.1, 1.), border_policy='reflect'):
        ...
        随机方向移动性模型。该模型基于随机行走。
        飞行长度是从均匀分布中选择的，最小值为0，最大值设置为最大尺寸值。
        速度也是从均匀分布中选择的，边界由速度参数设置。如果设置了wt_max，则从值介于0和wt_max之间的均匀分布中选择等待时间。
        如果未设置wt_max，则等待时间将设置为“无”
        ...
        keyword arguments:
          ...
          *wt_max*:最大等待时间
            Double, maximum value for the waiting time distribution.等待时间分布的最大值。
            If wt_max is set, the waiting time is chosen from a uniform distribution with values between 0 and wt_max.
            If wt_max is not set, the waiting time is set to None.
            Default is None.
            如果设置了wt_max，则从值介于0和wt_max之间的均匀分布中选择等待时间。
            如果未设置wt_max，则等待时间将设置为“无”。
            默认值为“无”。
          ...
          *border_policy*:
            ...
            If 'reflect', the node reflects off the border.
            If 'wrap', the node reappears at the opposite edge (as in a torus-shaped area).
            字符串，可以是“reflect”或“wrap”。节点到达边界时使用的策略。
            如果“reflect”，则节点会反射出边界。
            如果“wrap”，节点将重新出现在相对的边（如在圆环形区域中）。
        '''
