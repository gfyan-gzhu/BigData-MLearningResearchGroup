...
model = AutoModelForSequenceClassification. \
    from_pretrained(checkpoint, num_labels=10, ignore_mismatched_sizes=True)

device = torch.device("cuda")
model.to(device)

data_file = 'THUTT_AllNews_96W.csv'
DATA = pd.read_csv(data_file, index_col=0)
DATA = DATA.sample(500_000)
# set training data sizes
split_index = int(DATA.shape[0] * 0.50)  # or 1, 0.75, 0.25
...
tokenized_dataset = NewsTitles.map(tokenize_function, batched=True, remove_columns=['title'])
