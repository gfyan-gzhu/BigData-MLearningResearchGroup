...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.lstm = nn.LSTM(config.cnn_filter,
                            config.hidden_size,
    ...
    def forward(self, x):
        x = x.permute(0, 2, 1) # 32, 10, 8 => 32, 8, 10
        x = self.conv1d(x)
        x = self.maxpool(x) # 32, 32, 10
        x = x.permute(0, 2, 1) # 32, 10, 32
        ...
        # 设置lstm最初的前项输出
        h_0 = torch.randn(lstm_layers * self.config.num_directions, batch_size, self.config.hidden_size).to(device)
